...
parser.add_argument('--architecture', type=str, default='resnet', help='Architecture name of the DNN model.')
parser.add_argument('--dataset', type=str, default='cifar10', help='In-distrubution dataset of the pretrained model.')
# Loading details
...
if 'resnet' == args.architecture:
    mean = (0.4914, 0.4822, 0.4465)
    std = (0.2023, 0.1994, 0.2010)
else:
    assert 'densenet' == args.architecture
...
if 'cifar10' == args.dataset:
    train_data = datasets.CIFAR10('/data/share/ood_datasets/cifar', train=True, transform=test_transform, download=True)
    test_data = datasets.CIFAR10('/data/share/ood_datasets/cifar', train=False, transform=test_transform, download=True)
    num_classes = 10
else:
    train_data = datasets.CIFAR100('/data/share/ood_datasets/cifar', train=True, transform=test_transform, download=True)
    test_data = datasets.CIFAR100('/data/share/ood_datasets/cifar', train=False, transform=test_transform, download=True)
    num_classes = 100
...
for i in range(ood_num_examples * args.num_to_avg):
    ood_data[i] = gaussian(ood_data[i], sigma=1.5, multichannel=False)
...
ood_data = datasets.ImageFolder("/data/share/ood_datasets/Mahalanobis/data/LSUN_resize",
                            transform=transforms.Compose([transforms.Resize(32), transforms.CenterCrop(32),
                                                   transforms.ToTensor(), transforms.Normalize(mean, std)]))
ood_loader = torch.utils.data.DataLoader(ood_data, batch_size=args.test_bs, shuffle=True,
                                         num_workers=args.prefetch, pin_memory=True)
...
ood_data = datasets.ImageFolder("/data/share/ood_datasets/Mahalanobis/data/Imagenet_resize",
                            transform=transforms.Compose([transforms.Resize(32), transforms.CenterCrop(32),
                                                   transforms.ToTensor(), transforms.Normalize(mean, std)]))
ood_loader = torch.utils.data.DataLoader(ood_data, batch_size=args.test_bs, shuffle=True,
                                         num_workers=args.prefetch, pin_memory=True)
...
if 'cifar10' == args.dataset:
    ood_data = datasets.CIFAR100('/data/share/ood_datasets/cifar', train=False, transform=test_transform)
else:
    ood_data = datasets.CIFAR10('/data/share/ood_datasets/cifar', train=False, transform=test_transform)

ood_loader = torch.utils.data.DataLoader(ood_data, batch_size=args.test_bs, shuffle=True,
                                         num_workers=args.prefetch, pin_memory=True)


print('\n\nCIFAR-100 Detection') if 'cifar10' == args.dataset else print('\n\nCIFAR-10 Detection')
get_and_print_results(ood_loader)
