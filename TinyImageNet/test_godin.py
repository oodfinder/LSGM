...
from models.wrn import WideResNetx
...
parser.add_argument('--method_name', '-m', type=str, default='wrn_godin', help='Method name.')
# Loading details
parser.add_argument('--layers', default=40, type=int, help='total number of layers')
parser.add_argument('--widen-factor', default=2, type=int, help='widen factor')
parser.add_argument('--droprate', default=0.3, type=float, help='dropout probability')
parser.add_argument('--load', '-l', type=str, default='./godin_pretrained', help='Checkpoint path to resume / test.')
...
# Create model
underlying_net = WideResNetx(args.layers, num_classes, args.widen_factor, dropRate=args.droprate)

if args.ngpu > 0:
    underlying_net = underlying_net.cuda()
...
if args.ngpu > 0:
    h = h.cuda()
net = DeconfNet(underlying_net, underlying_net.output_size, num_classes, h, baseline)
...
# Find best noise magnitude for godin
validation_size = 2000
data_list = []
for batch_idx, (data, target) in enumerate(train_loader):
    if batch_idx >= validation_size // args.test_bs:
        break
    data = data.cuda()
...
for magnitude in m_list:
    ...
    for data in data_list:
        batch_score = godin_score(net, data, magnitude)
        # print(batch_score.mean())
        tmp_list.append(batch_score.mean())
    score_of_magnitude = sum(tmp_list) / len(tmp_list)
    score_list.append(score_of_magnitude)
    print(f'{magnitude}: {score_of_magnitude}')

best_index = int(np.array(score_list).argmax()) # int() to avoid warning
best_magnitude = m_list[best_index]
print('best_magnitude:', best_magnitude)
...
def get_ood_scores(loader, in_dist=False):
    ...
    for batch_idx, (data, target) in enumerate(loader):
        ...
        godin_scores = -godin_score(net, data, noise_magnitude=best_magnitude)
        _score.append(godin_scores)
        ...
        if in_dist:
            ...
            _right_score.append(godin_scores[right_indices])
            _wrong_score.append(godin_scores[wrong_indices])
